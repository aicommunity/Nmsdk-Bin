...
## 2026-09-12 — Хрупкость фиксированного порога (диагноз)

Синхронизация длин на PASS AsymRm достаточна для last-pulse; узкое место — скалярный максимум потенциала зоны vs близкие чужие паттерны. Эталон ~480 мс **не** робастнее по зазору (~6.3e-6). Probe B: tip/ResistanceMin на Done Test не расширяют зазор (потолок ~3.3e-4 тихий @50, ниже критерия 5e-4). Фиксы модели нейрона / зоны длительной потенции **исключены**; конфиг-фикс амплитудного запаса не найден. Документ: [`THR_FRAGILITY_DIAG.md`](../StructTrain/SelectivityAsymRm/THR_FRAGILITY_DIAG.md).
