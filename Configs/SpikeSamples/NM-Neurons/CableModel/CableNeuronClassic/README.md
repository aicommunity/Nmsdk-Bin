## CableNeuronClassic — классическое сравнение кабельной модели с LIF и точечной моделью

**Путь:** `Bin/Configs/SpikeSamples/NM-Neurons/CableModel/CableNeuronClassic`
**Статус валидации:** VALID (см. `Reports/SpikeSamples-Validation-Report.md`)

### Назначение конфигурации

Эта конфигурация демонстрирует **классическое сравнение кабельной модели нейрона** (`NPulseNeuronCable`) с моделью Leaky Integrate-and-Fire (LIF, `NPulseNeuronIaF`) и биологически реалистичной точечной моделью (`NSPNeuronBio`). Все три модели получают одинаковый входной сигнал, что позволяет сравнивать их реакции и выявлять особенности кабельной модели.

Согласно [2], кабельная модель учитывает пространственное распространение потенциала по дендритам и соме, что отличает её от точечных моделей. Данная конфигурация позволяет изучить эти различия на практике и сравнить с классическими моделями нейронов.
...
```mermaid
flowchart TD
    Generator[PGenerator<br/>NPGenerator] --> CableNeuron[CableNeuron3<br/>NPulseNeuronCable<br/>Кабельная модель]
...
**Кабельный нейрон:**
- **CableNeuron3** (`NPulseNeuronCable`) — кабельная модель нейрона
...
- **PNeuron** (`NSPNeuronBio`) — биологически реалистичный точечный нейрон
  - Более детальное описание мембранных процессов по сравнению с простой LIF моделью
  - Структура: 1 соматический участок, без дендритов
...
- Биологически реалистичная точечная модель (`NSPNeuronBio`) — более детальное описание мембранных процессов
...
2. **Изучение влияния частоты входной стимуляции:**
   ...
   - Выявление различий в поведении между простой LIF моделью и более сложными моделями
...
- **LIF модель:** должна демонстрировать более простую динамику по сравнению с кабельной и биологически реалистичной точечной моделями
- **Биологически реалистичная модель:** должна демонстрировать более сложную динамику по сравнению с LIF моделью
