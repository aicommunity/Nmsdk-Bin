#!/usr/bin/env bash
# Parallel Train then Test for SelectivityFastSpan grid.
set -euo pipefail
...
run_wave train "$TRAIN_T" || true

# sync train -> test structure/weights
for exp in "${EXPS[@]}"; do
  neuron=$(awk -F'\t' -v e="$exp" '$1==e{print $2;exit}' "$META")
  "$COPY" sync "$ROOT/$exp/Train" "$ROOT/$exp/Test" || true
...
def get_tag(text,tag):
    m=re.search(rf"<{tag}\b[^>]*>([^<]*)</{tag}>", text); return m.group(1) if m else None
test=set_tag(test,"NeuronClassName",neuron)
flt=get_tag(train,"FixedLTZThreshold")
if flt: test=set_tag(test,"FixedLTZThreshold",flt)
# keep SyncTolerance from scaled setup
tp.write_text(test,encoding="utf-8")
PY
done

echo "=== TEST wave MAX_JOBS=$MAX_JOBS t=$TEST_T ==="
run_wave test "$TEST_T" || true
...
for exp in "${EXPS[@]}"; do
  ...
  if [[ -f "$EVAL" ]]; then
    line=$(python3 "$EVAL" "$csv" 2>/dev/null || echo "")
  else
    line=""
  fi
