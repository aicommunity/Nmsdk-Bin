...
| **Рецепт tip-resistance при Rmin** | Сопротивления кончиков `2e7 2e7 2e7 8.6e7`, пол `ResistanceMin=2e7`, затем порог по тихому зонду. Часто вместе с ёмкостью 1e9 на коротких паттернах — «рецепт PHASE5/6» |
| **Канон Rmin** | Тот же фиксированный вектор `2e7 2e7 2e7 8.6e7` (пост-шаг, не результат подбора обучением) |
| **Плоский tip-resistance 8.6e7×4** | Все четыре кончика `8.6e7` (AsymRm на 25 мс) |
| **Tip-resistance на Done** | Значения сопротивлений с завершённого обучения (вектор **уникален** для каждого Train) |
...
**Итог на ~480 мс (классический):** переход от старого фиксированного порога (~0.0115) к **рецепту PHASE6** (канон Rmin **или** Done + новый порог по тихому зонду) поднимает 4→7 из 8. Отдельно замена сопротивлений на канон Rmin против Done **не** даёт выигрыша (оба 7 из 8). Подробнее о том, какие векторы стоят на диске — §4.7.
...
|-------------------|----------|
| На **25 мс** в Test стоит **плоский** tip-resistance `8.6e7×4` (не канон Rmin); точность **8 из 8**, спайк на цели да | FAIL-CSV «тот же Train, другой вектор» с N из 8 |
| На **50/100 мс** в Test — **канон Rmin** `2e7×3+8.6e7`; **8 из 8**, спайк на цели да | Чистая пара A/B только по tip-resistance без прочих правок |
| PHASE5: при «все 8.6e7» на длиннее 25 мс зазор цель−чужой по зоне часто отрицательный; при Rmin зазор > 0 | — |
...
### 4.7. Обобщаются ли значения tip-resistance?
...
| Класс | Вектор (нормализовано) | `ResistanceMin` | Где встречается у успешных |
|-------|------------------------|-----------------|----------------------------|
| **Канон Rmin** | `2e7 2e7 2e7 8.6e7` | `2e7` | Большинство: AsymRm 50/100; Branch 25/50 (+ nextseg/preinh); Branch 100 preinh/nextseg; Branch ~480 tiprmin; Phase6 tiprmin/preinh/twin; FastSpan C1e9; LtzCal twin 50/100 |
| **Плоский 8.6e7×4** | `8.6e7 8.6e7 8.6e7 8.6e7` | `2e7` | **AsymRm и LtzCal twin только на 25 мс** (8 из 8) |
| **Done (уникальный)** | зависит от Train | часто `2e7` | **Branch 100 мс gen** (+ клоны B/C): ≈`7.4e7 8.9e7 1.3e8 5.1e8` → 8 из 8; Phase6 `thr_only`: ≈`2.58e7 2.72e7 2.19e7 8.6e7` → 7 из 8 (как tiprmin) |

Канон Rmin — **фиксированный пост-шаг скрипта**, а не «оптимум, найденный C++». Done-вектор после обучения **каждый раз разный**.

#### Успешные по длительности паттерна × способу обучения

| Длительность | Способ обучения | Вектор на Test | Точность | Спайк на цели | Обобщение |
|--------------|-----------------|----------------|----------|---------------|-----------|
| 25 мс | Classic AsymRm / LtzCal twin | плоский `8.6e7×4` | 8 из 8 | да | **Не** канон Rmin |
| 25 мс | Branch gen / preinh / nextseg | канон Rmin | 8 из 8 | да | Канон |
| 25 мс | FastSpan C1e9 | канон Rmin | 8 из 8 (gen; cold Need=1) | да | Канон на диске |
| 50 мс | Classic AsymRm / LtzCal / FastSpan preinh | канон Rmin | 8 из 8 | да | Канон |
| 50 мс | Branch все рычаги | канон Rmin | 8 из 8 | да | Канон |
| 100 мс | Classic AsymRm / LtzCal / FastSpan | канон Rmin | 7–8 из 8 | да | Канон |
| 100 мс | Branch preinh / nextseg | канон Rmin | 8 из 8 | да | Канон |
| 100 мс | Branch **gen** | **Done** (один общий для A/B/C) | 8 из 8 | да | Исключение: Rmin давал 7 из 8 |
| ~480 мс | Classic Phase6 tiprmin | канон Rmin | 7 из 8 | да | Канон |
| ~480 мс | Classic Phase6 thr_only | Done ≈`2.58e7…8.6e7` | 7 из 8 | да | **Другой** вектор, **та же** точность |
| ~480 мс | Branch tiprmin / nextseg | канон Rmin | 8 из 8 | да | Канон |
| ~480 мс | Branch preinh tiprmin | канон Rmin | 7 из 8 | да | Канон |

#### Ответ

1. **Обобщается процедура (канон Rmin)**, а не набор «лучших сопротивлений, найденных обучением». Один и тот же вектор `2e7 2e7 2e7 8.6e7` стоит на Test у успешных classic (AsymRm≥50), Branch (кроме gen@100), FastSpan C1e9 и Phase6 tiprmin — на 50 / 100 / ~480 мс.
2. **Не один вектор на все успешные:**
   - AsymRm/LtzCal **25 мс** — плоский `8.6e7×4`;
   - Branch **100 мс gen** — Done-вектор (общий для пакетов A/B/C этого эксперимента, но **не** канон Rmin и не плоский base).
3. **Done не обобщается между экспериментами:** у Branch100 gen, Phase6 thr_only и legacy Branch — **разные** четвёрки чисел. На ~480 мс classic канон Rmin и Done thr_only дают **одинаковые** 7 из 8 → «наилучший» вектор там **не уникален**.
4. Между способами обучения (classic vs Branch vs FastSpan) при каноне Rmin вектор **один**; отличаются порог (зона vs сома) и результат cold, не сами сопротивления кончиков.

```mermaid
flowchart TD
  post["Пост-шаг tip-resistance"]
  post --> rmin["Канон 2e7 x3 + 8.6e7"]
  post --> flat["AsymRm25: 8.6e7 x4"]
  post --> done["Done: уникальный вектор Train"]
  rmin --> okMost["PASS большинство span и learner"]
  flat --> ok25["PASS только AsymRm и LtzCal 25"]
  done --> okBr100["PASS Branch100 gen; Phase6 thr_only = tiprmin по точности"]
```
