#!/usr/bin/env bash
# Train + verify Done+calibrate + sync + Test for AsymRmLtzCalBranch.
set -euo pipefail
...
LOG="$GRID/run_asymrm_ltzcal_branch.log"
...
[[ -f "$META" ]] || { echo "Run setup_asymrm_ltzcal_branch.sh first" >&2; exit 1; }
mapfile -t EXPS < <(awk -F'\t' 'NR>1{print $1}' "$META")

verify_model() {
  local model="$1" neuron="$2" kind="$3" cap="$4" exc_rm="$5" exc_rsyn="$6"
  python3 - "$exc_rm" "$exc_rsyn" <<'PY' > /tmp/asym_br_inh.txt
import sys
print(f"{float(sys.argv[1])*10:g}")
print(f"{float(sys.argv[2])*10:g}")
PY
  local inh_rm inh_rsyn
  inh_rm=$(sed -n '1p' /tmp/asym_br_inh.txt)
  inh_rsyn=$(sed -n '2p' /tmp/asym_br_inh.txt)
  local args=(--expect-dissoc 0.001 --expect-cap "$cap" --expect-secr 0.001
    ...
    --expect-class "$neuron" --min-count 1)
  if [[ "$kind" == *preinh* ]]; then
    args+=(--expect-inh 2.5)
  else
    args+=(--no-inh)
...
run_wave() {
  ...
  for exp in "${EXPS[@]}"; do
    ...
    (
      cd "$dir"
      if [[ "$mode" == train ]]; then
        "$NM" -c Project.ini -s -t "$tlim" -x -S >run_console.log 2>&1
      else
        rm -rf SelectivityLog
        "$NM" -c Project.ini -s -t "$tlim" -x >run_console.log 2>&1
      fi
    ) &
...
if (( fail_done )) && [[ "${ALLOW_PARTIAL_TRAIN:-0}" != "1" ]]; then
  echo "Abort: Branch train not Done/calibrated"
  exit 1
fi

echo "=== VERIFY elements + SYNC ==="
for exp in "${EXPS[@]}"; do
  ...
  verify_model "$GRID/$exp/Train/Model_00.xml" "$neuron" "$kind" "$cap" "$exc_rm" "$exc_rsyn" || true
  echo "sync $exp"
...
print(exp, acc_s, mode, "gate", gate, "FixedLTZ", flt, "Cal", cal)
PY
done
echo "Wrote $OUT"
cat "$OUT"
