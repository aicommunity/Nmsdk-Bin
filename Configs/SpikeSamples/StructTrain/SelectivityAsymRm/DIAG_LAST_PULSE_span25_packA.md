...
## Фаза 5 — span 50 / 100 (2026-09-12)

Полный лог + таблица робастности: [`PHASE5_SPAN50_100.md`](PHASE5_SPAN50_100.md).

- **TipR@Rmin** `2e7×3 + 8.6e7` обязателен на 50/100 (base×4 → negative gap).
- packA_gen @50/@100 PASS; preinh A + LtzCal twin (clone **Test**, GTS=20000) PASS; B/C gen clone→C1e9 PASS.
- Twin ловушка: `inject_analyzer` из грязного Train → лишние tip connectors / fire_all.

### Робастность thr vs span25

| | 25 мс | 50 мс | 100 мс |
|--|-------|-------|--------|
| gap tgt−foil | ~1.5e-4 | ~1.7e-4 | **~5e-6** |
| характер thr | хрупкий | **тот же порядок хрупкости** | **на порядок хуже** |
| модели (gen/preinh/twin/B/C) | per-exp thr; preinh ниже | thr общий после clone | thr общий; gap одинаковый |

Вывод: PASS на разных длительностях/моделях — это **робастный перенос рецепта и last-pulse**, не широкий порог. Амплитудный запас @50 ≈ @25; @100 — лезвие. Причины: схождение чужого и целевого максимума потенциала зоны на длинной интеграции; TipR у минимума чинит знак зазора, но не его ширину; clone моделей не меняет физику зазора.

Last-pulse PASS **не отменяет** хрупкость фиксированного порога: цель отвечает в конце паттерна, но окно между максимумом зоны на цели и на самом трудном чужом остаётся узким. Эталон ~480 мс ещё уже по зазору (~6e-6), при этом без режима «ответ на всех». Разбор: [`THR_FRAGILITY_DIAG.md`](THR_FRAGILITY_DIAG.md).

Фаза 5 waves 0–2 **закрыты**. Preinh B/C и автостарт всей сетки — отдельно.
